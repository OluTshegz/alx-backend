...
class LRUCache(BaseCaching):
    """
    LRUCache defines a caching system that follows
    the LRU (Least Recently Used) algorithm.
    """

    def __init__(self):
        """
        Initialize the LRUCache instance by calling
    ...
    def put(self, key, item):
        ...
        if key is not None and item is not None:
            ...
            if len(self.cache_data) > BaseCaching.MAX_ITEMS:
                # Identify the least recently used key
                # (the first key in access_order)
                lru_key = self.access_order.pop(0)

                # Remove the least recently used key from the cache_data
